...
const CreateVnetModal = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/vnetsCreate", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subscriptionId, rgName, vnetName, location }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Błąd tworzenia VNetu");
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <label>Nazwa VNetu:</label>
        <input
          type="text"
          value={vnetName}
          onChange={e => setVnetName(e.target.value)}
          placeholder="np. my-vnet-prod"
        />
        ...
        <div style={{ marginTop: "20px" }}>
          <button onClick={handleSubmit} disabled={loading || !subscriptionId || !rgName || !vnetName}>
...
export default CreateVnetModal;
