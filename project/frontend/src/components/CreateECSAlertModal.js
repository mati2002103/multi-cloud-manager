...
const CreateECSAlertModal = ({ isOpen, onClose, onCreated, region, clusterName, serviceName, metricsList }) => {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const first = metricsList?.[0]?.type || "";
    setMetricType(first);
    const prefix = `ecs-${clusterName}-${serviceName}-`;
    setAlertName(`${prefix}${first || "metric"}-gt-${threshold}`);
    setError(null);
  }, [isOpen, clusterName, serviceName, metricsList]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSubmit = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch("/api/aws/ecs/create-alert", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        <h2>🚨 Utwórz alert (ECS): {serviceName}</h2>

        <label>Nazwa alertu:</label>
        <input type="text" value={alertName} onChange={(e) => setAlertName(e.target.value)} style={styles.input} />
        ...
        <input type="number" value={threshold} onChange={(e) => setThreshold(Number(e.target.value))} style={styles.input} />
...
export default CreateECSAlertModal;
